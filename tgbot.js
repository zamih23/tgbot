...
bot.onText(/\/start/, (msg) => {
  ...
  let oneUserBank = copy(linkBank); //arr for everyone user
   bot.sendMessage(chatId, 'Hi, '+ msg.chat.first_name + '. I`m BadHumorBot. Welcome! What do you want?',{
  reply_markup: JSON.stringify({
    ...
    userDataBase.push({'uid': chatId, 'firstTime': '16:44', 'secondTime': '16:45', 'thirdTime': '16:46', 'fourthTime': '16:47', 'bank': oneUserBank, 'regularySending': false });
...
bot.on('callback_query', (query) => {
  ...
  if (query.data === '4') {
    ...
    bot.sendPhoto(userDataBase[index]['uid'], 'https://drive.google.com/uc?export=view&id=' + userDataBase[index]['bank'][rand]['id']   , {                //link to photo
   reply_markup: JSON.stringify({
     inline_keyboard: [
...
else if (query.data === '5') {           //regulary sending photos
  ...
  for(let i = 0; i < userDataBase.length; i++ ) {
    let rand = getRandom(0,userDataBase[i]['bank'].length);
     ...
     if((userDataBase[i]['firstTime']||userDataBase[i]['secondTime']||userDataBase[i]['thirdTime']||userDataBase[i]['fourthTime'] === curDate) && userDataBase[i]['regularySending'] == true ){

       bot.sendPhoto(userDataBase[i]['uid'], 'https://drive.google.com/uc?export=view&id=' + userDataBase[i]['bank'][rand]['id']  ).catch(function(error) {
